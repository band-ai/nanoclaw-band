    ...
    for await (const event of query.events) {
      ...
      if (event.type === 'init') {
        ...
        if (event.text && userVisibleToolUsed) {
          // Band seam: the agent already sent a user-visible MCP message this
          // turn; the SDK's final text is a terse tool-result summary. Log it,
          // don't re-deliver, and record the exchange as completed.
          log(`[tool-result] ${event.text.slice(0, 200)}`);
          // Upstream's one-door task delivery still applies on this path: the
          // final text belongs in the run log even though it is not delivered
          // to a channel.
        ...
        // Band seam: reset for the next turn within this open query.
