...
// Band seam: the label carries the display name and the kind of destination
// (channel type, e.g. "Band"/"Telegram", or "agent") so the model can map a
// user's natural reference ("send it on Band") to the right local name.
// Kept over upstream's destinationLabel(): the ordering, the "agent" marker and
// the capitalised channel type are what Band's prompts are tuned against.
// Consolidating the two is INT-1203, not this merge.
function destLabel(d: DestinationEntry): string {
  const parts: string[] = [];
