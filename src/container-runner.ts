...
interface ActiveContainer {
  ...
  stopReason?: string;
  /** Spawn wall-clock time — host-sweep's absolute idle ceiling falls back to
   *  this when no heartbeat file exists yet (see getContainerStartedAtMs). */
  startedAtMs: number;
}
...
async function spawnContainer(session: Session): Promise<void> {
  ...
  activeContainers.set(session.id, { process: container, containerName, state: 'running', startedAtMs: Date.now() });
