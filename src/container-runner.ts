...
// Band seam: keeps the fork's name (its caller and provider-container-registry's
// doc comment reference resolveContainerContribution) and stays async because the
// body awaits the channel-scoped contribution. Upstream's provider-only
// resolveProviderContribution is the same function minus that channel hop.
